        ...
        <div class="AnaDiv">
            ...
            <div class="icerik">
                <%
                    ...
                    if request.form("Ara")="" or request.form("Ara")=" " then
                        response.write("<h2>Lütfen Arama Alanını Boş Bırakmayınız</h2>")
                    else
                        if request.form("Ara")<>"" then
                            Set oConn = Server.CreateObject("ADODB.Connection")
                            oConn.Open("DRIVER={Microsoft Access Driver (*.mdb)}; DBQ=" & Server.MapPath("database.mdb"))
                            ssql="select * from Kullanıcı_Bilgileri where Ad+' '+Soyad='" & request.form("Ara")& "';"
                ...
                %>
                         <table style="margin-left: 20px;margin-top: 20px;margin-right: 10px;" border="1" style="width: 100%" class="tablo">
                        <tr>
                            <th>Ad</th>
                            <th>Soyad</th>
                            <th>Yaş</th>
                ...
                <%
                    session("KullanıcıAdSoyadTut")=Ara("Ad")+" "+Ara("Soyad")
                    Ara.movenext
                    loop
                    end if
                   end if
                ...
                <%
                    if (session("UserLoggedIn")="Admin" or session("UserLoggedIn")="admin") and session("KullanıcıAdSoyadTut")<>"" then
                %>
                    <form action="KisiBilgiDuzenle.asp" method="post"><input type="submit" value="Kullanıcı Bilgilerini Düzenle" name="Duzenle" style="float: right;margin-top: 5px;margin-right: 30px"></form>
                    <form action="KullaniciSil.asp" method="post"><input type="submit" value="Kullanıcıyı Sil" name="sil" style="float: right;margin-top: 5px;margin-right: 10px"></form>
                <% end if %>  
